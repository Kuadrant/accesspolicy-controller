...
This document outlines the tasks needed to build the `AccessPolicy`-to-`AuthPolicy` standalone controller. The focus is exclusively on `Gateway` targetRefs and a gateway-aware translation layer that maps abstract MCP authorization rules into Kuadrant's `AuthPolicy` by employing CEL macro substitution and combining rules.
...
- [ ] **Task 1.1: Initialize Project**
  ...
  - Scaffold the `AccessPolicy` API or import it from the upstream `kube-agentic-networking` repository.
- [ ] **Task 1.2: Dependencies & RBAC**
  ...
  - Add `+kubebuilder:rbac` markers for `GET/LIST/WATCH/UPDATE` on `AccessPolicy` and `Gateway` resources.
...
- [ ] **Task 3.1: Watch Configuration**
  - Setup the primary watch on `AccessPolicy`.
...
- [ ] **Task 3.2: Reconcile Loop & Policy Combination**
  - Find all `AccessPolicy` resources targeting the specific `Gateway`.
  ...
  - Update the status conditions of all successfully processed `AccessPolicies` to `Programmed = True` or `Accepted = False` based on CEL validation.
...
- [ ] **Task 4.3: E2E Integration Tests**
  - Assert that multiple `AccessPolicies` targeting the same Gateway result in a **single** `AuthPolicy`.
  - Assert that applying an `AccessPolicy` correctly programs the translated predicate.

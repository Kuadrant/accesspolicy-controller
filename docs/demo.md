...
```bash
# Create cluster
kind create cluster --name xaccesspolicy-demo
...
## 2. Deploy the XAccessPolicy Controller
Build and load the controller image:

```bash
make install # Installs the XAccessPolicy CRDs
make docker-build IMG=xaccesspolicy-controller:demo
kind load docker-image xaccesspolicy-controller:demo --name xaccesspolicy-demo
make deploy IMG=xaccesspolicy-controller:demo
```

## 3. Deploy an MCP Server
Deploy a sample MCP server that exposes tools like `get-sum` and `echo`.
*(We assume you have built an `xaccesspolicy-mcp-server:demo` image loaded into Kind.)*

...
# ─────────────────────────────────────────────────────────────────────────────
# Step 11: Apply policy resources (Gateway + HTTPRoute + AccessPolicy)
# ─────────────────────────────────────────────────────────────────────────────
step "Applying Gateway & AccessPolicy"
...
success "Gateway, HTTPRoute, and AccessPolicy applied"
...
info "Waiting for AccessPolicy to be accepted..."
sleep 3
...
# Start port-forward on port 8080 in background
kubectl port-forward svc/demo-gateway-istio ${PORT_FORWARD_PORT}:8080 -n quickstart-ns >/dev/null 2>&1 &
PF_PID=$!
sleep 2

if kill -0 "${PF_PID}" 2>/dev/null; then
  success "Port-forward active (PID ${PF_PID}) on port ${PORT_FORWARD_PORT}"
else

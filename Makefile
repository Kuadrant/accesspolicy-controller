...
test: manifests generate fmt vet setup-envtest ## Run tests.
	KUBEBUILDER_ASSETS="$(shell "$(ENVTEST)" use $(ENVTEST_K8S_VERSION) --bin-dir "$(LOCALBIN)" -p path --verify=false)" go test $$(go list ./... | grep -v /e2e) -coverprofile cover.out
...
KIND_CLUSTER ?= accesspolicy-test-e2e
...
docker-build: test ## Build docker image with the manager.
	CGO_ENABLED=0 GOOS=linux GOARCH=$(shell go env GOARCH) go build -o bin/manager cmd/main.go
	docker build -t ${IMG} .
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name accesspolicy-builder
	$(CONTAINER_TOOL) buildx use accesspolicy-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm accesspolicy-builder
	rm Dockerfile.cross
